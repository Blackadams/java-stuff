...
public class MainController {
    ...
    public List<StateAndRef<InsuranceTokenType>> getIOUs() {
        // Filter by state type: IOU.
        return proxy.vaultQuery(InsuranceTokenType.class).getStates();
    }
...
//    }
@PutMapping(value =  "/issue-iou" , consumes = APPLICATION_JSON_VALUE, produces = APPLICATION_JSON_VALUE )
public ResponseEntity<String> issueIOU(@RequestParam(value = "RegNo") String RegNo,
                                       @RequestParam(value = "Make") String make,
                                       @RequestParam(value = "Model") String model,
                                       @RequestParam(value = "Mileage") long mileage,
                                       @RequestParam(value = "Price") long price,
                                       @RequestParam(value = "Issuer") Party issuer,
                                       @RequestParam(value = "Observers") List<Party> observers)  throws IllegalArgumentException {
    //This api method will fail because the state is not implemented.
//        // Get party objects for myself and the counterparty.
        Party me = proxy.nodeInfo().getLegalIdentities().get(0);
        Party notary = proxy.wellKnownPartyFromX500Name(TokenTypeConstants.NOTARY);
//        Party lender = Optional.ofNullable(proxy.wellKnownPartyFromX500Name(CordaX500Name.parse(party))).orElseThrow(() -> new IllegalArgumentException("Unknown party name."));
        ...
        try {
            InsuranceTokenType state = new InsuranceTokenType(Collections.singletonList(me),
                    new UniqueIdentifier(), RegNo, make, model, mileage, price, issuer);
            final TransactionState<InsuranceTokenType> txState = new TransactionState<>(state, notary);
            final CreateEvolvableTokens created = new CreateEvolvableTokens(txState, observers);
            // Start the IOUIssueFlow. We block and waits for the flow to return.
            SignedTransaction result = proxy.startTrackedFlowDynamic(CreateTokenFlow.class, created).getReturnValue().get();
...
//    @PutMapping(value =  "/create-tokens" , produces = TEXT_PLAIN_VALUE)
//    public ResponseEntity<String> CreateToken(@RequestParam(value = "techdomain") Party techdomain,
//                                              @RequestParam(value = "RegNo") String RegNo,
//                                              @RequestParam(value = "Make") String make,
//                                              @RequestParam(value = "Model") String model,
//                                              @RequestParam(value = "Mileage") long mileage,
//                                              @RequestParam(value = "Price") long price,
//                                              @RequestParam(value = "Issuer") Party issuer) throws IllegalArgumentException {
//        // Get party objects for myself and the counterparty.
//        Party me = proxy.nodeInfo().getLegalIdentities().get(0);
////        Party receiver = Optional.ofNullable(proxy.wellKnownPartyFromX500Name(CordaX500Name.parse(issuer))).orElseThrow(() -> new IllegalArgumentException("Unknown party name."));
//        // Create a new IOU state using the parameters given.
//        try {
//            InsuranceTokenType state = new InsuranceTokenType(Collections.singletonList(techdomain),
//                    new UniqueIdentifier(), RegNo, make, model, mileage, price, issuer);
//            // Start the IOUIssueFlow. We block and waits for the flow to return.
//            SignedTransaction result = proxy.startTrackedFlowDynamic(IssueTokenFlow.class, state).getReturnValue().get();
//            // Return the response.
//            return ResponseEntity
//                    .status(HttpStatus.CREATED)
//                    .body("Transaction id "+ result.getId() +" committed to ledger.\n " + result.getTx().getOutput(0));
...
//        try {
//            proxy.startTrackedFlowDynamic(SpendTokenFlow.SpendTokenFlowInitiator.class, linearId, newLender).getReturnValue().get();
...
//        UniqueIdentifier linearId = new UniqueIdentifier(null, UUID.fromString(id));
//        try {
//            proxy.startTrackedFlowDynamic(RedeemTokenFlow.class, linearId,
